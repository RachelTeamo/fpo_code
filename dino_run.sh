 CUDA_VISIBLE_DEVICES=0,1 torchrun --nproc_per_node=2 --master_port=2452 train.py \
  ...
  --proj-coeff=0.5 \
  --encoder-depth=4 \
  --output-dir="inter_exps_div" \
  --exp-name="linear-dinov2-enc4_ratio06_temp01" \
...
torchrun --nnodes=1 --nproc_per_node=2 generate.py \
  ...
  --ckpt ./inter_exps_div/inear-dinov2-enc4_ratio06_temp01/checkpoints/0400000.pt \
  --sample-dir ./inear-dinov2-enc4_ratio06_temp01 \
  --path-type=linear \
  --encoder-depth=4 \
...
--sample_batch /home/user/code/zty/cvpr26/working/fisher_repa/inter_exps_div/inear-dinov2-enc4_ratio06_temp01/SiT-B-2-0400000-size-256-vae-ema-cfg-1.0-seed-0-sde.npz
